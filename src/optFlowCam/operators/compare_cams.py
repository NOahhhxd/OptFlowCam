...
def create_cams(object_mesh, matrix, min_scale, max_scale, is_earth_cam=False):
    ...
    for i in range(2):
        ...
        pos,view =  random_spherical_pos(lookat_pos, lookat_normal, matrix, dist)
        view_n = view.normalized()
        scale = view.length / focal
        cams.append(create_cam(pos, view_n, get_up(view_n).normalized(), scale, focal))
        """
        if view_n[1] < 0:
